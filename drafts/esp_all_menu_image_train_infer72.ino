...
// FULL VISION ML  — v72
//
// v72 additions over v71 (dynamic menu for variable numClasses):
//   - myTotalItems changed from compile-time const int=5 to a runtime
//     #define macro: (myCfg.numClasses + 2).  The menu now adapts
//     automatically when numClasses is set by config.json — collect
//     items occupy slots 1..numClasses, Train=numClasses+1,
//     Infer=numClasses+2, WebStream=numClasses+3 (serial-only shortcut).
//   - myDispatchByIndex(int idx) helper added — single function that maps
//     any menu index to the correct action for any numClasses value.
//     Replaces three copies of hardcoded if/else chains (number-key,
//     L-key, and touch dispatch) that previously assumed exactly 3 classes.
//   - Forward declaration for myDispatchByIndex() added.
//   - STATUS response updated to print dynamic key assignments.
//   - Version bumped to esp32-on-device-72.
//
...
// myTotalItems is now dynamic: numClasses + 2 (Train + Infer).
// WebStream is serial-only (key after last infer), not in OLED menu.
// Use the inline accessor everywhere instead of a fixed constant.
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n=== XIAO ESP32-S3 ML System Starting (v72) ===");
...
// ─────────────────────────────────────────────────────────────────────────────
//  myDispatchByIndex — execute the action for a given menu index.
//  Collect items occupy indices 1..numClasses.
//  Train = numClasses+1, Infer = numClasses+2, WebStream = numClasses+3 (serial only).
//  This replaces three copies of hardcoded if/else chains so the menu adapts
//  automatically when numClasses changes via config.json.
// ─────────────────────────────────────────────────────────────────────────────
void myDispatchByIndex(int idx) {
  if (idx >= 1 && idx <= myCfg.numClasses) {
    myActionCollect(idx - 1);
  } else if (idx == myCfg.numClasses + 1) {
    myActionTrain();
  } else if (idx == myCfg.numClasses + 2) {
    myActionInfer();
  } else if (idx == myCfg.numClasses + 3) {
    myActionWebStream();
  }
}
...
void myDispatchMenuCmd(const String& cmd) {
  ...
  if (cmd.length() == 1) {
    ...
    if (c >= '1' && c <= '9') {
      ...
      if (!myIsSelected) {
        int newIndex = c - '0';
        if (newIndex <= myTotalItems + 1) {  // +1 allows WebStream shortcut key
          myMenuIndex = min(newIndex, myTotalItems);
          myIsSelected = true;
          myLastActivityTime = now;
          myDispatchByIndex(newIndex);  // newIndex may be numClasses+3 = WebStream
        }
      }
      return;
    }
...
void myHandleStringCommand(const String& cmd) {
  ...
  // ── STATUS (for serial monitor / health check) ───────────
    ...
    Serial.println("OK:=== esp32-on-device-72 Status ===");
    ...
    Serial.printf("OK:Menu      : 1-%d=collect  %d=Train  %d=Infer  %d=WebStream  t=next  l=select\n",
